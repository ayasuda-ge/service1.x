...
class ECApi extends ECCommon {
	...
	let _dbg = this._debug,_this=this;
        return this.cmd('agt', ['-hsh', '-smp'], {
            ...
            //_dbg(`${new Date()} EC: ${_this._options["info"]["id"]} _getRefHash > obj.stdout: ${obj.stdout} hsh:${hsh}`);
	    return obj.stdout;
	});
    }
	...
	*/
	...
	//commented for eliminating sac    
	/*
	_this._getBearerToken(_this._options["info"]["cid"],
			      _this._options["info"]["csc"],
		...
		.catch(err=>{
		    _this._debug(`${new Date()} EC: ${process.env.ZONE} ECApi > _getBearerToken. bearer token renewal failed. err: ${err}. programme exiting.`);
		    process.exitCode = 1;
		    process.exit();
		});
