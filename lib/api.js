...
class ECApi extends ECCommon {
	...
	*/
	...
	_this._getBearerToken(_this._options["info"]["csc"],
		...
		.catch(err=>{
		    _this._debug(`${new Date()} EC: ${process.env.ZONE} ECApi > _getBearerToken. bearer token renewal failed. _this._options["info"]: ${_this._options["info"]}err: ${err}. programme exiting.`);
		    process.exitCode = 1;
		    process.exit();
		});
