...
class ECApi extends ECCommon {
	...
	this._getBearerToken(_this._options["info"]["csc"],
			     _this._options["info"]["cid"],
			     _this._options["user-api-auth"]["authUrl"])
		.then((body)=>{
		    this._options["sacTkn"]=body["access_token"];
		    this._debug(`${new Date()} EC: ${process.env.ZONE} ECApi > _getBearerToken. bearer token renewed successfully.`);
		})
		.catch(err=>{
		    this._debug(`${new Date()} EC: ${process.env.ZONE} ECApi > _getBearerToken. bearer token renewal failed. err: ${err}`);
		});
	    
	setInterval(()=>{
	    this._debug(`${new Date()} EC: ${process.env.ZONE} bearer token refreshing.`);
	    ...
	    this._getBearerToken(_this._options["info"]["csc"],
			...
			.then((body)=>{
			    this._options["sacTkn"]=body["access_token"];
         		    this._debug(`${new Date()} EC: ${process.env.ZONE} ECApi > _getBearerToken. scheduled bearer token renewed successfully.`);
			})
			.catch(err=>{
			    this._debug(`${new Date()} EC: ${process.env.ZONE} ECApi > _getBearerToken. scheduled bearer token renewal failed. err: ${err}`);
			});
	}, 1000*60*18);    
    }
	...
	*/
	...
	return new Promise((reso, reje) => {
		...
		if (nb < 1 ) {
			return reje("failed in account serialisation");
		}			
		return reso(acc.GetGroupsDetail());
