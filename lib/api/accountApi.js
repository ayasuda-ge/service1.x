...
class ECAccountApi extends RSAuth {
	...
	try {
		...
		this._p = PATH.normalize(req.url).replace(/^(\.\.[\/\\])+/, '').replace("/v1/admin/accounts/","").replace("/v1.1/admin/accounts/","");
	...
	return new Promise((reso,reje)=>{
		...
		req.on('end', ()=>{
			...
			debugger;
			_debug(`${new Date()} EC: ${process.env.ZONE} Invalid json parsing e: ${e} for ${req.url} (EC Internal API)`);
			return reje(e);
		    }
			...
			return reje("account update failed");
		    }
		    
		    return reso({code:201,data:{status: "account updated."}});
		    
		});
		
	    }).catch((obj)=>{
		_debug(`${new Date()} EC: ${process.env.ZONE} authentication failed for ${req.url} obj: ${JSON.stringify(obj)}`);
		return reje('failed authentication');			
	    });
	...
	return new Promise((reso,reje)=>{
		...
		if (req.url.indexOf("/add")>0){
		    ...
		    if (!_ref){
			return reje("account group creation failed, please verify the groupId/agentId.");
		    }
			
		    return reso({code:201,data:{status: "account generated/added.",details:_ref}});		    
		}
		
		let _chunk='',_body;
		    
		//_debug(`${new Date()} EC: ${process.env.ZONE} createAccount > validate > account validated (EC Internal API)`);
		...
		req.on('end', ()=>{
	           ...
	           //_debug(`${new Date()} EC: ${process.env.ZONE} createAccount > validate > data end`);
		   ...
		   debugger;
		    ...
		    let _sp=acc.CreateAccountGroup(_zon);
	           ...
	           //_debug(`${new Date()} EC: ${process.env.ZONE} createAccount > validate > data end > CreateAccountGroup > _sp ${_sp}`);

	    	   if (!_sp){
			_debug(`${new Date()} EC: ${process.env.ZONE} account creation failed for ${req.url} (EC Internal API)`);
			return reje("account group creation failed");
		    }

		    return reso({code:201,data:{status: "account group created."}});
	...
	return new Promise((reso,reje)=>{
		...
		if (!_s){
		    _debug(`${new Date()}  ${process.env.ZONE} account _p:${_p} does not exist for ${req.url} (EC Internal API)`);
		    return reje("account does not exist");
		}
		
		return reso({code:200,data:_s});
		
	    }).catch((obj)=>{
		_debug(`${new Date()} EC: ${process.env.ZONE} authentication failed for ${req.url} (EC Internal API)`);
		return reje(`failded obj: ${obj}`);
	    });
	});	
    }
